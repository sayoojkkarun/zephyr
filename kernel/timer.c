...
void z_timer_expiration_handler(struct _timeout *t)
{
	 ...
	 * that is handled here will not be pending at this point. But it may
	 * happen that before this function is executed and interrupts are
	 * locked again, a given timer gets restarted from an interrupt context
	 * that has a priority higher than the system timer interrupt. Then, the
	 * timeout structure for this timer will be pending again. In such case,
	 * since the timer was restarted, its expiration handler should not be
	 * executed, so the function exits immediately.
	 */
